...
public class Methods_basic implements recordMaintenance 
{
    ...
    private void createTable()//creates a new table
    {
        try 
        {
            final String newTable = "create table ts_districts(id varchar(255) unique, name varchar(255), zone varchar(255), totalCases int, newCasesDay int, deathsDay int, dischargedDay int, totalDeaths int, totalDischarged int, activeCases int)";
    ...
    private void insertData(Data d)//inserts data into table from csv file
    {
        try
        {
            final String fillTable="insert into ts_districts values(?,?,?,?,?,?,?,?,?,?)";
    ...
    public void tableformation()//initiates table formation and insertion
    {
        ...
        try {
            BufferedReader reader=new BufferedReader(new FileReader("C:\\Users\\DELL\\Desktop\\Covid Statistics Record Maintenance\\Covid Statistics Record Maintenance\\bin\\ts_districts.csv"));
    ...
    public void display_data()//displays all data
    {
        ...
        final String SQL="select * from ts_districts";
    ...
    {
        final String SQL="insert into ts_districts values(?,?,?,?,?,?,?,?,?,?)";
        try(PreparedStatement stmt=con.prepareStatement(SQL))
        {
    ...
    {
        final String SQL="delete from ts_districts where id=?";
        try(PreparedStatement stmt=con.prepareStatement(SQL))
        {
            stmt.setString(1,args[1]);
            stmt.executeUpdate();
    ...
    {
        final String SQL="update ts_districts set totalCases=?, newCasesDay=?, deathsDay=?, dischargedDay=?, totalDeaths=?,totalDischarged=?, activeCases=? where id=?";
        try(PreparedStatement stmt=con.prepareStatement(SQL))
        {
